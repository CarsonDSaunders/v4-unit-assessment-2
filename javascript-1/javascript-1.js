...
for (let i = 0; i < lettersToPair.length; i++) {
    for (let j = 0; j < lettersToPair.length; j++) {
        if (lettersToPair[i] === lettersToPair[j] && i != j) {
            if (lettersToPair[i] != 'Boop') {
                pairsArray.push([i,j]);
                lettersToPair[i] = 'Boop'
            }
        }
    }
}
...
//CODE HERE
let phone1 = new Phone('Google', 'Pixel 3AXL', 9001, 'Blue', false);

let phone2 = new Phone('Apple', 'iPhone 420', 0.25, 'Ugly', false);

let phone3 = new Phone('Samsung', 'Note 27XL Pro',  128, 'Galaxy Black', false);

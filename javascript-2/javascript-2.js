...
saleProducts = products.map(element => {
  let reducedPrice = {};
  Object.assign(reducedPrice, element);
  reducedPrice.price = element.price * .75;
  return reducedPrice;
})
////////////////////PROBLEM 3////////////////////
/*
...
let blueProducts = saleProducts.filter(element => element.color.includes('blue'))
...
let orderTotal = blueProducts.reduce((acc, element) => element.price + acc, 0);
...
const person = {
  name: 'Danielle',
  age: 18,
  jobs: ['older sister', 'violin teacher', 'chemical engineer'],
  birthday: function() {
    this.age += 1},
  favorites: {
    color: 'yellow',
